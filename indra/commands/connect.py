...
def check_wg_interface(interface):
    ...
    try:
        subprocess.run(["wg", "show", interface], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
        return True
...
def setup_wireguard_interface():
    ...
    if not check_wg_interface(WG_INTERFACE):
        ...
        try:
            subprocess.run(["wireguard.exe", "/installtunnelservice", wg_config_file], check=True)
